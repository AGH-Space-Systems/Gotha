...
uint8_t GY85_Init(GY85 *imu, I2C_HandleTypeDef *hi2c)
{
    ...
    if (HAL_I2C_Mem_Write(hi2c, ADXL345_ADDR, 0x2D, 1, &data, 1, 100) != HAL_OK)
        return 0;
    uint8_t check;
    ...
    if (HAL_I2C_Mem_Write(hi2c, ITG3200_ADDR, 0x15, 1, &data, 1, 100) != HAL_OK)
        return 0;

    data = 0x00;
    if (HAL_I2C_Mem_Write(hi2c, ITG3200_ADDR, 0x3E, 1, &data, 1, 100) != HAL_OK)
        return 0;
